...
class ContextIO(object):
    ...
    def request_uri(self, uri, method="GET", params={}, headers={}, raw=False):
    ...
    def post_connect_token(self, callback_url, **params):
        params = Resource.sanitize_params(params, ['service_level', 'email'])
        params['callback_url'] = callback_url
        if not params.get('service_level', None):
            params['service_level'] = 'PRO'
...
class Resource(object):
    ...
    def request_uri(self, uri_elems, method="GET", params={}, raw=False):
        uri = self.uri_for(uri_elems)
        return self.parent.request_uri(uri, method=method, params=params, raw=raw)
...
class Account(Resource):
    ...
    def get_file(self, file_id, raw=True):
        return self.request_uri('files/%s/content' % file_id, raw=raw)
    ...
    def get_thread(self, thread_id):
        return self.request_uri('threads/gm-' + thread_id)
    
    def get_message(self, message_id):
        obj = self.request_uri('messages/' + message_id)
    ...
    def get_messages(self, **params):
        ...
        ret = []
        for obj in self.request_uri('messages', params=params):
            try:
                ret.append(Message(self, obj))
            except Exception, e: # F*** it. Something's screwy w/ this message.
                                 # Log and move on.
                logging.warning("Unable to process message. Carry on.")
                info = "".join(traceback.format_tb(sys.exc_info()[2]))
                logging.warning(info)
                logging.warning(str(e))
                try:
                    logging.warning(obj)
                except: # Something weird with object encoding
                    try:
                        logging.warning(obj.encode('utf-8'))
                    except:
                        logging.warning("Unable to log obj contents")
        return ret
    
    def get_messages_for_contact(self, contact_email, **params):
        params = Resource.sanitize_params(params, ['offset', 'limit'])
        uri = 'contacts/%s/messages' % quote(contact_email)
        return [Message(self, obj) for obj in self.request_uri(uri, params=params)]
...
class File(Resource):
    ...
    def __init__(self, parent, defn):
        ...
        if 'person_info' in defn:
            person_info, to, cc, frm = process_person_info(parent, defn['person_info'], defn['addresses'])
    ...
    def get_content(self, raw=True):
        return self.request_uri('content', raw=raw)
...
class Message(Resource):
    keys = ['body', 'headers', 'date', 'subject', 'addresses', 'files', 'message_id', 'email_message_id', 'gmail_message_id', 'gmail_thread_id', 'person_info', 'date_indexed', 'folders']
    body = None
    flags = None
    headers = None
    thread = None
